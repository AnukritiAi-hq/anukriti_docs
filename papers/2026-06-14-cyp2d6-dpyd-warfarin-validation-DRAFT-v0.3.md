...
The honest reading of this result is not that the classifier failed, but that a single classifier is the wrong tool for two distinct regimes. We therefore adopt a **two-tier inference** architecture. For variants with clinical labels or measured activity — the regime the classifier was trained for — the functional classifier applies (decreased_function F1 = 0.66). For novel population-discovery variants lacking both — exactly the regime that population-specific sequencing produces, and the one most relevant to ancestry-equity — the raw AlphaMissense pathogenicity score is the primary actionable signal, used directly rather than through the classifier. The two confirmed-pathogenic Scaria variants illustrate the boundary: c.704G>A (missense) carries AlphaMissense 0.93, well above the published likely-pathogenic threshold (0.564) and directly actionable; c.1970delC, a frameshift, has no AlphaMissense score by construction, and is correctly handled by the deterministic loss-of-function logic rather than either probabilistic tool. This separation keeps each method within the evidence regime where it is valid — consistent with the platform's broader refusal to manufacture certainty where the data do not support it.
